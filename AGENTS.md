...
- 多供应商 Provider 架构（未发版，下一版 0.1.56）：`src/providers/` 每供应商一文件（`opencode.js`/`openrouter.js`/`dispatcher.js`/`model-id.js`），统一接口 `{id, chat(body), listModels(), preheat, close}`；模型 id 前缀路由——**默认供应商（opencode）裸 id 向后兼容**，其它供应商带 `<provider>/` 前缀（如 `openrouter/google/gemma-4-26b-a4b-it:free`），dispatcher 按前缀路由并把 `body.model` 剥回原始 id 再转发；OpenRouter provider：免费模型=`pricing.prompt/completion 全 0`（实测 417 中 20 个免费，匿名可拉 `/api/v1/models`），chat 必须有 `MSLXDFF_OPENROUTER_KEY`（匿名 401），发 `HTTP-Referer`+`X-Title` 品牌头，`MSLXDFF_OPENROUTER_BASE_URL` 可覆盖（测试/代理用）；`createModelsService` 支持 `providers` 数组时聚合各自 `listModels()`（单供应商取数失败不影响整体）；bin 装配：插件 `createUpstream` 仍是整体替换式单通道（保留），无插件时默认 opencode + 有 key 才加载 openrouter。新增测试 `test/providers-*.test.js`。