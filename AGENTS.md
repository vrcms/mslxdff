...
- 上游免费池限流与锁定：opencode.ai 免费额度 429 会静默 failover 换模型，`x-mslxdff-model-lock` 可锁定；`src/upstream.js` 遇 free 模型 public 429 会匿名 hermes 重试（`MSLXDFF_FREE_ANON_RETRIES=3`/`DELAY=1000`，UA=opencode 才过，`big-pickle`/`mimo` 需此 UA），命中记 `free-anon-extra.txt`。
...
- Keep-Alive + 预热：`undici.Agent(keepAlive 30s/60s, connections 20)` 显式复用，`srv.ready` 100ms 后预热 `GET /zen/v1/models`；`MSLXDFF_PREHEAT=0` 可关；`UndiciFetch || fetch` 必须配对，否则多测试并发 `fetch failed`，测试需 `await client.close()`。
...
- 对话终端 `-chat`（0.1.58）：`src/chat/` 6 文件均 <10KB（config/store/tools/prompt/upstream/repl），严格仅 `mimo-v2.5-free` 优先/`big-pickle` 兜底，模糊匹配由大模型查 `cli_help_mini`+实时模型列表完成；工具仅 `run_command`（仅拦 `-uninstall`）与 `read_file`（限 `pkgRoot`/`logDir`/`stateDir`）；历史 `~/.config/mslxdff/chat-history.json`（`MSLXDFF_CHAT_HISTORY` 可覆盖），超 18000 字符保留 `system`+最近 8 条、其余发 mimo 摘要成 `【历史摘要】`；`maybeCompress` 曾因返回原引用致 `messages.length=0` 清空触发 400，已修为拷贝。
