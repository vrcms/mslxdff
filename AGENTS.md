...
- 流式生成超时中断（0.1.29）：`GEN_TIMEOUT_MS` 默认 20s（env `MSLXDFF_GEN_TIMEOUT_MS`）——单个模型整体流式（TTFB+生成+relay）超过该值即使已开始输出也主动 `res.end()` 中断，客户端拿到明确 EOF 而非无限卡死（半截内容不会被接管成另一模型，已写字节无法干净切换）；relay 返回 `{status,ttfMs,totalMs,aborted,interrupted}`，`interrupted` 触发该模型记录 slow + `slow-model` 事件打 `interrupted:true`。注：首块超时 `STREAM_TIMEOUT_MS`（25s，未写字节）才走 failover 切换；`GEN_TIMEOUT_MS` 是已开始输出的整体上限，只中断不换模型。
