...
describe("C6 provider-row 纯函数", () => {
  ...
  test("US2 format 展示", () => {
    const row = { id: "openrouter", enabled: true, baseUrl: "https://openrouter.ai/api/v1", keys: ["sk-1234567890"], allowed: ["a", "b", "c"], authCount: 0, note: "" };
    const s = formatProviderRow(row);
    assert.match(s, /● openrouter/);
    assert.match(s, /已启用/);
    assert.match(s, /sk-…7890/);
    assert.match(s, /allow 3/);
    assert.match(s, /共享 借出/);
    assert.match(s, /https:\/\/openrouter/);
  });

  test("US3 workbuddy 桩检测", () => {
    const row1 = { id: "workbuddy", enabled: true, baseUrl: "https://copilot.tencent.com", keys: ["k-new"], allowed: [], authCount: 0, note: "" };
    ...
    assert.match(s1, /未启用/);
    const row2 = { id: "workbuddy", enabled: false, baseUrl: "http://127.0.0.1:8080", keys: ["sk-real-123456789012345"], allowed: [], authCount: 1, note: "测试桩 (key=k-new, baseUrl=127.0.0.1) — 请重跑" };
